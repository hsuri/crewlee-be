...
**Multi-restaurant employees** (`POST /api/auth/login`): since `email` is scoped per-restaurant, not globally unique (see `users` above), the same person can hold a `users` row at more than one restaurant. Login tries the submitted password against every active row for that email. If exactly one matches, that's a normal login — the response's `user` also carries `restaurantId`/`restaurantName`, plus `otherRestaurants` (names only, no tokens) if the email has other active accounts elsewhere, purely so the frontend can hint "you have another account, log in there separately." If the *same* password matches more than one row (person reuses one password across jobs), returning any single one arbitrarily would silently log them into the wrong restaurant — instead the response is `{"accounts": [{token, user}, ...]}`, one already-valid token per matching restaurant, and the frontend shows a picker (`login.js`'s `showAccountPicker`) instead of guessing. `set_password` has the mirror-image known limitation: it always claims the *oldest* still-unclaimed row for an email, so two restaurants both inviting the exact same email before either invite is claimed leaves the second one stuck pending with no way to disambiguate from the signup form alone.
